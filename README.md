# MCP Prepare Page - Cloud Onboarding

Interactive single-page mockup for the Managed Cloud Platform (MCP) onboarding journey.
...
#### Linking to documentation (optional)
The UI can open external resources if URLs are provided in the config:
- Section (tab) action buttons:
  - `section.content.documentationUrl` → used by the **Open documentation** button
  - `section.content.resourcesUrl` → used by the **Related resources** button
- Phase quick links:
  - `quickLinks[].url` → opens when clicking a Quick Link
